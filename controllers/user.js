...
const REDIRECT_URI = `${process.env.BASE_URL}/redirect`;
...
exports.redirect = (req, res) => {
  ...
  cca.acquireTokenByCode(tokenRequest).then((response) => {
    console.log('\nResponse: \n:', response);
    ssn.userInfo = response;
    console.log('Success! You are logged in');
    res.redirect(req.session.returnTo || '/candidates');
  }).catch((error) => {
...
exports.logout = async (req, res) => {
  ...
  const browserURL = `${req.protocol}://${req.headers.host}${req.baseUrl}`;
  req.user = null;
  req.session.destroy();
  req.logout();
  res.send(`https://login.microsoftonline.com/common/oauth2/v2.0/logout?post_logout_redirect_uri=${browserURL}`);
};
